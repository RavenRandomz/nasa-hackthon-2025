...
func calculate_storage_area(mass_dict: Dictionary, areaType: String) -> float:
	...
	for item in mass_dict.keys():
		...
		#print_debug(itemVol)
		var rawArea = total_volume / ROOM_HEIGHT_CONSTANT
	...
	return food_storage_area if areaType.match("food_area") else total_area
...
func _ready():
	...
	var storage_area = round(storage_volume / ROOM_HEIGHT_CONSTANT * 100.0) / 100.0
	...
	var total_area = round(total_nhv + storage_volume * ROOM_HEIGHT_CONSTANT * 100.0) / 100.0
